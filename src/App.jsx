...
function App() {
  ...
  async function handleImportEnvironment() {
    ...
    try {
      const importedEnvironment = await importEnvironmentFromFile(new File([content], 'environment.json', { type: 'application/json' }))
      setDialogState({ open: true, type: 'choice', title: 'Import environment', message: 'Choose how to add the environment.', initialValue: 'merge', options: [{ label: 'Merge into existing environments', value: 'merge' }, { label: 'Replace current environments', value: 'replace' }], confirmLabel: 'Import', cancelLabel: 'Cancel', onConfirm: (mode) => { setDialogState((current) => ({ ...current, open: false })); const shouldReplace = mode === 'replace'; const nextEnvironments = shouldReplace ? [importedEnvironment] : [...environments, { ...importedEnvironment, active: false }]; handleEnvironmentsChange(nextEnvironments) }, onCancel: () => setDialogState((current) => ({ ...current, open: false })) })
    } catch (error) {
      setDialogState({ open: true, type: 'confirm', title: 'Import failed', message: error?.message || 'Import failed.', initialValue: '', options: [], confirmLabel: 'OK', cancelLabel: '', onConfirm: () => setDialogState((current) => ({ ...current, open: false })), onCancel: () => setDialogState((current) => ({ ...current, open: false })) })
    }
  }
