...
function getVariableValue(environment, key) {
  if (!environment || !Array.isArray(environment.variables)) return undefined

  const normalizedKey = key.trim()
  const variable = environment.variables.find((item) => (
    item?.enabled !== false && String(item.key ?? '').trim() === normalizedKey
  ))
